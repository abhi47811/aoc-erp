    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-elevation-xs">
        ...
        <div className="p-4 space-y-4">
          ...
          <a
            ...
            className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-lg text-sm font-medium transition-colors ${waUrl && message ? 'bg-green-600 hover:bg-green-700 text-white cursor-pointer' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
          >
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-elevation-xs">
        ...
        {!mLoading && msgList.length === 0 && (
          <div className="py-10 text-center text-sm text-slate-400">No message history</div>
        )}
        {msgList.length > 0 && (
                ...
                {msgList.map((m: any) => (
                  <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-800">
                      ...
                      <div className="text-xs text-slate-400">{m.phone_number}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-slate-400 text-xs">{new Date(m.created_at).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
